...
from utils.dataset_utils import DenoiseTestDataset, DerainDehazeDataset, DeblurTestDataset
from utils.val_utils import AverageMeter, compute_psnr_ssim
from utils.image_io import save_image_tensor
from net.model_3D import R2R, R2RLocal
...
def test_Deblur(net, dataset, task="deblur", interact_label=None):
    ...
    with torch.no_grad():
        for ([degraded_name], degrad_patch, clean_patch) in tqdm(testloader):
            ...
            restored = patch_inference(degrad_patch, net, tile=None,  interact_label=None)
...
def heatmap(data):
    ...
    fig, ax = plt.subplots()
    im = ax.imshow(data_np,cmap='RdBu_r', aspect="auto", vmin=0.0, vmax=1.0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_path', type=str, default="output/3D/", help='output save path')
    parser.add_argument('--ckpt_name', type=str, default="train_ckpt_3D_finetune/", help='checkpoint save path')
    parser.add_argument('--prompt_dir', type=str, default="save_prompts_3D/", help='prompt save path')
