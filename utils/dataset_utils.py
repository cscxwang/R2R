...
class PromptTrainDataset(Dataset):
    ...
    def _merge_ids(self):
        ...
        if "denoise_15" in self.de_type:
            ...
            print(f"denoise samples={str(len(self.sample_ids))}" )
        if "derain" in self.de_type:
            self.sample_ids+= self.rs_ids
            print(f"denoise + derain samples={str(len(self.sample_ids))}")
        
        if "dehaze" in self.de_type:
            self.sample_ids+= self.hazy_ids
            print(f"denoise + derain + dehaze samples={str(len(self.sample_ids))}")
...
class PromptTrainDataset5D(Dataset):
    ...
    def _get_light_name(self, lol_name):  # ------ get high
